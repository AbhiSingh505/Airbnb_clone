        ...
        NavigationStack {
            VStack(spacing: 0) {
                ...
                if viewModel.isShowingResults {
                    SearchResultsView(viewModel: viewModel, namespace: searchAnimation)
                } else {
...
struct SearchResultsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            SearchResultsContent(viewModel: viewModel, namespace: namespace)
            Spacer()
        }
        ...
        VStack(spacing: 16) {
            ...
            HStack {
                ...
                Button {
                    ...
                    Image(systemName: viewModel.isListening ? "mic.fill" : "mic")
                        .foregroundColor(viewModel.isListening ? .red : Theme.textSecondary)
                }
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) {
                        ForEach(viewModel.suggestedNeighborhoods, id: \.self) { hood in
                            Button {
                                viewModel.searchText = hood
                                viewModel.performSearch(query: hood)
                            } label: {
                                Text(hood)
...
struct SearchResultsContent: View {
    ...
    var body: some View {
        if viewModel.isLoading {
            ...
            SearchResultsGrid(viewModel: viewModel, namespace: namespace)
        }
    }
}
        ...
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(viewModel.searchResults) { card in
                    NavigationLink {
                        CardsDetailView(cardId: card.id, matchedNamespace: namespace)
                            .toolbar(.hidden, for: .tabBar)
                    } label: {
                        SearchResultCard(card: card, namespace: namespace)
                    }
                }
            }
            .padding(.horizontal, 16)
